...
// SVG processor
const loadSVG = function (svgFile) {
  const createDOMPurify = require('dompurify')
  const { JSDOM } = require('jsdom')
  const window = (new JSDOM('')).window
  const DOMPurify = createDOMPurify(window)
  return DOMPurify.sanitize(svgFile, {USE_PROFILES: {svg: true, svgFilters: true}})
  // const window = require('svgdom')
  // const SVG = require('svg.js')(window)
  // const draw = SVG(svgFile)
...
exports.renderAdmin = function (req, res, next) {
  ...
  Q.all(OFFICE_IDS.map(function (officeId) { return Q.nfcall(fs.readFile, path.join(global.dirMaps, officeId + '.svg')) }))
    .then(function (svgs) {
      global.logger.log('debug', 'renderAdmin called')
      const context = {
...
exports.test = {
  ...
  async get (id, params) { return Date.now() },
