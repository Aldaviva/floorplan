...
app.set('port', global.wwwPort)
app.set('views', path.join(__dirname, '/../views'))
app.set('view engine', 'hbs')
app.use(compression())
app.use(favicons(path.join(global.dirPublic, 'icons')))
...
app.use(cors())
app.use(slash(true, { base: global.mountPoint })) // prepends a base url to the redirect
app.use(function (err, req, res, next) {
  console.error(err.stack || err.message)
  res.type('text')
  res.send(500, err.message)
})
app.use(require('connect-less')({ src: global.dirPublic }))
app.use(express.static(global.dirPublic, { maxAge: 4 * 60 * 60 * 1000 }))
global.router.use(global.mountPoint, function (req, res, next) {
...
http.createServer(app).listen(app.get('port'), function () {
  console.log('Express server listening on port ' + app.get('port') + ' expecting callback to ' + global.mountPoint)
})
