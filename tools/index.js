...
function getFirstResultPhotoSrc (source) {
  const matches = source.match(/,"imageUrl":"(.+?)",/)

  if (matches) {
    const thumbnailSrc = matches[1]
    // const fullSizeSrc = thumbnailSrc.replace(/media\.licdn\.com\/mpr\/mpr\/shrink_60_60/, 'm.c.lnkd.licdn.com/media');
    const fullSizeSrc = 'http://m.c.lnkd.licdn.com/media' + thumbnailSrc
    return fullSizeSrc
  } else {
    return null
  }
...
function findProfileIdinSource (source) {
  const matches = source.match(/\/profile\/view\?id=(\d+)&authType=NAME_SEARCH/)
  if (matches) {
    return matches[1]
  } else {
...
function findTitleInSource (source) {
  // const matches = source.match(/"([^"]+) at Blue Jeans Network/);
  const matches = source.match(/"title_highlight":"(.+?)"/)
  if (matches) {
    return matches[1]
  } else {
    return null
...
function main () {
  names.forEach((fullname) => {
    const resultsUrl = getResultsPath(fullname)
    getSource(resultsUrl, function (resultsSource) {
      const person = {}
      person.fullname = fullname
      person.linkedInId = findProfileIdinSource(resultsSource)

      const photoPath = 'photos/' + fullname + '.jpg'
      if (!fs.existsSync(photoPath)) {
        const photoUrl = getFirstResultPhotoSrc(resultsSource)
        photoUrl && saveUrlToDisk(photoUrl, photoPath)
      }

      if (person.linkedInId) {
        const profileUrl = getProfilePath(person.linkedInId)
  ...
  /* names.forEach(function(fullname){
    const filePath = "photos/"+fullname+'.jpg';
    if(!fs.existsSync(filePath)){
      const resultsUrl = getResultsPath(fullname);
      getSource(resultsUrl, function(source){
        const photoUrl = getFirstResultPhotoSrc(source);
  ...
  /* names.forEach(function(fullname){
    const resultsUrl = getResultsPath(fullname);
    getSource(resultsUrl, function(source){
      const profileId = findProfileIdinSource(source);
      profileId && console.log(profileId, fullname);
    });
  }); */

  /* const remaining = merged.length;
  ...
  merged.forEach(function(person){
    if(person.linkedInId){
      ...
      getSource(profileUrl, function(profileSource){
        ...
        const title = findTitleInSource(profileSource);
        if(title){
          person.title = title;
        }
        if(!(--remaining)){
